"""
Calcualtes minima and maxima for all variables of the numpy arrays
...
for (i,arr) in enumerate(arr_gen):
    ...
    # double check if new minima and new maxima is smaller and bigger 
    # (or eaqual) respectivelly
    if np.any(minimum<new_minimum) or np.any(maximum>new_maximum):
        print("from min max, we have a problem")
        embed()

    minimum = deepcopy(new_minimum)
    maximum = deepcopy(new_maximum)
    
f = open('min_max7.pkl', 'wb')
