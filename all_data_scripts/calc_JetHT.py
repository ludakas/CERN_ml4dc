"""
Calcualtes minima and maxima for all variables (1st sweep over the data)
and then calculates histograms (2nd sweep over the data) of the numpy arrays
in the input directory eg. JetHT.
"""
...
paths = get_npy_paths('<PATH to JetHT directory>')
arr_gen = array_generator(paths)
print(len(paths))

n_sig = 0
n_back = 0
minimum = np.zeros(len(var_names_reduced5)*7)+np.inf
maximum = np.zeros(len(var_names_reduced5)*7)-np.inf
n_bins = 100
n_vars = len(minimum)
...
for (i,arr) in enumerate(arr_gen):
    ...
    if len(vals)>0:
        ...
        new_minimum = np.min([np.min(vals, axis=0), minimum], axis=0)
        new_maximum = np.max([np.max(vals, axis=0), maximum], axis=0)

    # if np.any(minimum<new_minimum) or np.any(maximum>new_maximum):
    #     embed()

    minimum = deepcopy(new_minimum)
    maximum = deepcopy(new_maximum)


for (i,arr) in enumerate(array_generator(paths)):
    if i%300==0:
        print(str(i)+' ', end="")
        sys.stdout.flush()
        if i%3000==0:
    ...
    # check if number of lumi-section is the same for all variables.
    ...
    if len(unique_sig)!=1 or len(unique_back)!=1:
        print(unique_sig, unique_back, i, ' JetHT, we have a problem')
        embed()
    
f = open('hists_JetHT.pkl', 'wb')
pickle.dump([hist_sig, hist_back, minimum, maximum], f)
f.close()

print('unique_sig',unique_sig)
print('unique_back',unique_back)
