"""
Count bad lumi-section and the reason they were removed.
a.) "removed_by_lumi" - removed because 'lumi' variable was 0
b.) "removed_other" - at least one of the chosen variables did not contain 7 values 
counting is done in modified get_filtered_events function.
"""
...
def get_filtered_events(array, var_list, verbose=False):
    """modified to count bad lumi-section"""
    n_vars = len(var_list)
    if verbose:
    ...
    removed_by_lumi = np.sum(np.logical_not(mask))
    ...
    removed_all = np.sum(np.logical_not(mask))
    removed_other = removed_all - removed_by_lumi
    ...
    return sigVals, backVals, np.array([removed_by_lumi, removed_other, removed_all])
...
n_back = 0
removed = np.zeros(3)
removed_per_file = np.zeros([3,60000])
for (i,arr) in enumerate(arr_gen):
    if i%300==0:
        print(str(i)+' ', end="")
        sys.stdout.flush()
        if i%3000==0:
            print('\n', n_sig, n_back, removed)
        
    sigVals, backVals, new_removed = get_filtered_events(arr, var_names_reduced5)
    n_sig += len(sigVals)
    n_back += len(backVals)
    removed += new_removed
    removed_per_file[:,i] = new_removed
    
f = open('removed.pkl', 'wb')
pickle.dump([n_sig, n_back,removed, removed_per_file], f)
f.close()
